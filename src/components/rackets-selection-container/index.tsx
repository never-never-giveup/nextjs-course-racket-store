...
export const RacketsSelectionContainer: FC<Props> = async ({ title, hrefToAll, className, getRackets }) => {
    ...
    return (<Selection title={title} hrefToAll={hrefToAll} className={className}>
        {rackets.map((product) =>
            <RacketSelectionItem
                key={product.id}
                id={`${product.id}`}
